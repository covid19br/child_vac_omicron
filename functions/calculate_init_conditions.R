...
library(Matrix)
library(matlib)
calculate_init_condition <- function(parameters){
  ## Definir estados
  ...
  result <- with(parameters,{
    ...
    T_mat.D <- Matrix(T_mat.D, sparse = T)

    result.D <- eigs(F_mat.D + T_mat.D, 1, retvec = T)
    ############################################################################
    ################################ OMICRON ###################################
    ############################################################################
    ...
    T_mat.O <- Matrix(T_mat.O, sparse = T)
    
    result.O <- eigs(F_mat.O+T_mat.O, 1, retvec = T)
    
    
    
    r.D <- Re(result.D$values[1])
    init.condit.D <- abs(Re(result.D$vectors[,1]))
    r.O <- Re(result.O$values[1])
    init.condit.O <- abs(Re(result.O$vectors[,1]))
    result <- list(init.condit.D = init.condit.D, init.condit.O = init.condit.O, r.D = log(r.D), r.O = log(r.O))
  })
  return(result)
}
